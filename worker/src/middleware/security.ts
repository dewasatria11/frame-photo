...
export const security = createMiddleware<AppEnv>(async (c, next) => {
  ...
  if (origin && isAllowed) {
    ...
    c.header("Access-Control-Allow-Headers", "Content-Type, X-File-Name");
    c.header("Access-Control-Max-Age", "86400");
  }
  if (c.req.method === "OPTIONS") {
    if (!isAllowed)
      return failure(c, "ORIGIN_NOT_ALLOWED", "Origin tidak diizinkan.", 403);
    return c.body(null, 204);
  }
  if (WRITE.has(c.req.method) && !isAllowed)
    return failure(c, "ORIGIN_NOT_ALLOWED", "Origin tidak diizinkan.", 403);
  ...
  if (WRITE.has(c.req.method)) {
    ...
    if (counters.size > 2000)
      for (const [k, v] of counters)
        if (v.reset < Date.now()) counters.delete(k);
